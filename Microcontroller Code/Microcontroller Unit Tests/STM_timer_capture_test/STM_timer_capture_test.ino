...
void InputCapture_IT_callback(void)
{
  ...
  if (CurrentCapture > LastCapture) {
    FrequencyMeasured = float(input_freq) / float(CurrentCapture - LastCapture);
  }
  else if (CurrentCapture <= LastCapture) {
    /* 0x1000 is max overflow value */
    FrequencyMeasured = float(input_freq) / float((0x10000 + CurrentCapture - LastCapture));
  }
...
void setup()
{
  ...
  MyTim->setOverflow(2500000, MICROSEC_FORMAT);
...
void loop()
{
  ...
  Serial.println((String)"Frequency = " + String(FrequencyMeasured, 6));
  delay(1000);
}
