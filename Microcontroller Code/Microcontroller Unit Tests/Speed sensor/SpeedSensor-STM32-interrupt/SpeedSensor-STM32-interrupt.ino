volatile int t_revs= -1; //counts total revolutions, ignores the first
  volatile int i_revs= -1;//counts last two revolutions
  //int curr_st;
  //int prev_st;
...
void setup(){
  ...
  attachInterrupt(digitalPinToInterrupt(signalPin),revs,RISING);
  Serial.begin(9600);
}
void loop() {
   if (i_revs==2){
